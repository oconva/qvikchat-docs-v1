...
Request body expects an object with `data` property. This `data` property should be assigned an object with the following properties:
...
For successful requests, the result object will contain:
...
For failed requests, the result object will contain:
